...
const getNotificationHref = (notification: Notification) => {
  ...
  if (notification.jobId && notification.eventType === "JOB_OFFER_RECEIVED") {
    return `/user/offers?jobId=${notification.jobId}`;
  }

  if (notification.jobId && (notification.eventType === "BOOKING_UPDATED" || notification.eventType === "BOOKING_COMPLETED")) {
    return `/user/repairs/${notification.jobId}`;
  }
...
export function UserNotificationPopover() {
  const router = useRouter();
  const queryClient = useQueryClient();
  const socket = useMemo(() => getSocketClient(), []);
  const { data: profileResponse } = useGetMyProfile();
  const userId = profileResponse?.data?.id;
  const notificationsKey = useMemo(() => getUserNotificationsQueryKey(userId), [userId]);
  const { data: notificationsResponse } = useUserNotifications(userId);
  ...
  useEffect(() => {
    ...
    const onNotification = (payload: ApiResponse<Notification> | Notification) => {
      ...
      if (!incoming || !userId || incoming.receiverUserId !== userId) {
        return;
      }
  ...
  }, [socket, queryClient, userId, notificationsKey]);
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    if (notification.eventType === "JOB_OFFER_RECEIVED" || notification.jobId) {
      await queryClient.refetchQueries({ queryKey: ["user-offers"], type: "all" });
    }

    if (notification.eventType === "OFFER_ACCEPTED") {
      await Promise.all([
