...
const ADMIN_EVENT_TYPES = new Set(["BOOKING_CONFIRMED", "JOB_POSTED", "WORKSHOP_REGISTERED"]);
...
const getNotificationHref = (notification: Notification) => {
  if (notification.eventType === "BOOKING_CONFIRMED" && notification.bookingId) {
    return `/admin/bookings/${notification.bookingId}`;
  }

  if (notification.eventType === "JOB_POSTED" && notification.jobId) {
    return `/admin/jobs/${notification.jobId}`;
  }

  if (notification.eventType === "WORKSHOP_REGISTERED" && notification.triggeredById) {
    return `/admin/workshops/${notification.triggeredById}`;
  }
...
const upsertNotification = (notifications: Notification[], incoming: Notification) => {
  ...
  return notifications.map((n) => (n.id === incoming.id ? { ...n, ...incoming } : n));
};
...
export function AdminNotificationPopover() {
  ...
  const adminUserId = profileResponse?.data?.id;
  const notificationsKey = useMemo(() => getUserNotificationsQueryKey(adminUserId), [adminUserId]);
  const { data: notificationsResponse } = useUserNotifications(adminUserId);
  const markNotificationAsRead = useMarkNotificationAsRead();

  // Only surface admin-relevant event types
  const notifications = useMemo(
    () =>
      asNotificationArray(notificationsResponse?.data).filter((n) =>
        ADMIN_EVENT_TYPES.has(n.eventType)
      ),
  ...
  const recentReadNotifications = useMemo(
    () => sortedNotifications.filter((n) => n.isRead).slice(0, 5),
    [sortedNotifications]
  );
  ...
  useEffect(() => {
    ...
    const onNotification = (payload: ApiResponse<Notification> | Notification) => {
      ...
      // Only handle admin-relevant event types
      if (!incoming || !ADMIN_EVENT_TYPES.has(incoming.eventType)) {
        return;
      }
  ...
  }, [socket, queryClient, adminUserId, notificationsKey]);
  ...
  const markOneAsRead = async (notificationId: string) => {
    ...
    queryClient.setQueryData<ApiResponse<Notification[]> | undefined>(notificationsKey, (data) => {
      ...
      return {
        ...data,
        data: asNotificationArray(data.data).map((n) =>
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    if (notification.eventType === "JOB_POSTED") {
      await queryClient.refetchQueries({ queryKey: ["admin-jobs"], type: "all" });
    }

    if (notification.eventType === "WORKSHOP_REGISTERED") {
      await queryClient.refetchQueries({ queryKey: ["admin-workshops"], type: "all" });
    }

    if (notification.eventType === "BOOKING_CONFIRMED") {
      await queryClient.refetchQueries({ queryKey: ["admin-bookings"], type: "all" });
    }
  ...
  const handleReadAll = async () => {
    ...
    await Promise.all(
      unreadNotifications.map((n) => markNotificationAsRead.mutateAsync(n.id))
    );
    ...
    queryClient.setQueryData<ApiResponse<Notification[]> | undefined>(notificationsKey, (data) => {
      ...
      return {
        ...data,
        data: asNotificationArray(data.data).map((n) => ({ ...n, isRead: true }))
      };
    });
  };
